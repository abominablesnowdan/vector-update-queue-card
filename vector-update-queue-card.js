class VectorUpdateQueueCard extends HTMLElement {
  ...
  icon(item) {
    ...
    return `/vector_ops_static/icons/${encodeURIComponent(key)}.svg`;
  }
  ...
  render() {
    ...
    const rows = available.map((item) => {
      ...
      return `<article class="update"><input class="pick" type="checkbox" data-select="${this.esc(id)}" ${this._selected.has(id) ? "checked" : ""} ${queued ? "disabled" : ""}><img src="${this.icon(item)}" alt="" onerror="this.onerror=null;this.src='/vector_ops_static/icons/service.svg'"><div class="identity"><strong>${this.esc(item.name)}</strong><small>${this.esc(version)}</small></div><span class="status ${status.toLowerCase().replaceAll(" ", "-")}">${status}</span>${reviewPanel}<div class="row-actions">${actions.join("")}</div></article>`;
    }).join("");
...
class VectorOpsOverviewCard extends HTMLElement {
  ...
  render() {
    ...
    const routes = allRoutes.filter((x) => this._filter === "all" || (this._filter === "ok" && routeHealthy(x)) || (this._filter === "bad" && !routeHealthy(x)));
    ...
    const now = new Date();
    const routeRows = routes.map((x) => { const auth = x.ok && [401,403].includes(Number(x.code)); const level = routeHealthy(x) ? "good" : auth ? "warn" : "bad"; return `<a class="service" href="${this.esc(this.safeUrl(x.url))}" target="_blank" rel="noopener"><img src="${this.icon(x.service_key)}" alt="" onerror="this.onerror=null;this.src='${this.icon("service")}'"><span><strong>${this.esc(x.name)}</strong><small>HTTP ${this.esc(x.code || "ERR")} · ${this.esc(x.ms)}ms${auth ? " · auth required" : ""}</small></span><span class="dot ${level}"></span></a>`; }).join("") || `<div class="empty">No matching services</div>`;
